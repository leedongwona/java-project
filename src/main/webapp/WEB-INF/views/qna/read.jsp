  ...
  <nav class="navbar navbar-expand-md navbar-dark fixed-top bg-dark">
      ...
      <form class="form-inline mt-2 mt-md-0">
		...
		<div class="container">
			...
			<div class="form-group row">
				<label for="id">작성자</label> <input class="form-control" value="${dto.id}" readonly>
			</div>

			<div class="form-group row">
				<label for="readcnt">조회수</label> <input class="form-control" value="${dto.readcnt}" readonly>
			</div>

			<div class="form-group row">
				<label for="regdate">작성일</label> <input class="form-control" value="${dto.regdate}" readonly>
			</div>

			<div class="form-group row">
				<label for="updatedate">수정일</label> <input class="form-control" value="${dto.updatedate}" readonly id="updatedate">
			</div>

			<div class="form-group row">
				<label for="title">제목</label> <input class="form-control" id="title" value="${dto.title}" readonly>
			</div>
			...
			<a href="/qna/list/${curpage}/${criteria}${empty keyword? '' : '/'}${keyword}" class="btn btn-link">목록</a>
			...
			<div class="modal-dialog">
				<div class="modal-content">
					...
					<div class="modal-body">
						<div class="form-group row">
							<label for="read_input_modal_pw">비밀번호</label> <input type="password" class="form-control" id="read_input_modal_pw">
						</div>

						<div class="form-group row">
							<label for="read_input_modal_title">제목</label> <input class="form-control" id="read_input_modal_title" value="${dto.title}">
						</div>
					...
					<div class="modal-footer">
						...
						<button id="read_btn_modal_submit" type="button" class="btn btn-primary">수정</button>
					</div>
				</div>
			</div>
		</div>
		...
		<div class="modal" tabindex="-1" id="read_div_replyui_update_modal" data-backdrop="static">
			<div class="modal-dialog">
				<div class="modal-content">
					...
					<div class="modal-footer">
						<button type="button" class="btn btn-secondary" data-dismiss="modal">취소</button>
						<button data-rno="" id="read_btn_replyui_update_modal_submit" type="button" class="btn btn-primary">수정</button>
					</div>
				</div>
			</div>
		</div>
	...
	<script type="text/javascript">
		...
		$(document).ready(function() {
			...
			getReplyAll2(bno, $("#reply_list"));
			
			$.getJSON("/qna/getFilenames?bno="+bno, function(result){
			...
			$("#read_btn_delete").click(function() {
				...
				$.ajax({
					...
					success : function(result) {
						if(result > 0){
							alert("삭제 성공");
							location.assign("/qna/list/${curpage}/${criteria}${empty keyword? '' : '/'}${keyword}");
						} else{
							alert("삭제 실패");
						}
					}
			...
			$("#read_btn_updateui").click(function() {
				location.assign("/qna/updateui/${curpage}/${criteria}${empty keyword?'':'/'}${keyword}${empty keyword?'':'/'}"+bno);
			});
